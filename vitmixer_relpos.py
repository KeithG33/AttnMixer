...
__all__ = ['VitMixerRelPos']  # model_registry will add each entrypoint fn to this
...
class VitMixerRelPos(nn.Module):
    """ Vision Mixer w/ Relative Position Bias on even layers only
    ...
    def __init__(
        ...
        # Determine token counts for alternating transposed layers
        num_patches = self.patch_embed.num_patches
        num_tokens = num_patches + self.num_prefix_tokens  # patches + optional cls token

        dpr = [x.item() for x in torch.linspace(0, drop_path_rate, depth)]  # stochastic depth decay rule
        # Alternate: even (normal, with relpos), odd (transposed, no relpos)
        blocks: List[nn.Module] = []
        for i in range(depth):
            dim_i = num_tokens if i % 2 == 1 else embed_dim
            heads_i = 1 if i % 2 == 1 else num_heads
            rel_pos_cls_i = rel_pos_cls if i % 2 == 0 else None
    ...
    def forward_intermediates(
        ...
        for i, blk in enumerate(blocks):
            if i % 2 == 1:
                # Transposed layer: process on [B, D, N] without relpos
                x = x.transpose(-2, -1)
                ...
                # Normal layer: use relpos if available
                if self.grad_checkpointing and not torch.jit.is_scripting():
    ...
    def forward_features(self, x):
        ...
        # Only apply relative position bias on even (normal) layers
        shared_rel_pos = self.shared_rel_pos.get_bias() if self.shared_rel_pos is not None else None
        for i, blk in enumerate(self.blocks):
            if i % 2 == 1:
                # Transposed layer: do not use relpos, operate on [B, D, N]
                x = x.transpose(-2, -1)
                if self.grad_checkpointing and not torch.jit.is_scripting():
                    x = checkpoint(blk, x)
                else:
                ...
                # Normal layer: apply relpos bias
                if self.grad_checkpointing and not torch.jit.is_scripting():
                    x = checkpoint(blk, x, shared_rel_pos=shared_rel_pos)
                else:
                    x = blk(x, shared_rel_pos=shared_rel_pos)
...
def _create_vitmixer_relpos(variant, pretrained=False, **kwargs):
    out_indices = kwargs.pop('out_indices', 3)
    model = build_model_with_cfg(
        VitMixerRelPos, variant, pretrained,
...
default_cfgs = generate_default_cfgs({
    # VitMixerRelPos configurations - no pretrained weights
    'vitmixer_relpos_pico_patch16_224': _cfg(url='', input_size=(3, 224, 224)),
    'vitmixer_relpos_tiny_patch16_224': _cfg(url='', input_size=(3, 224, 224)),
})




# VitMixerRelPos variants (mirroring VitMixer)
@register_model
def vitmixer_relpos_pico_patch16_224(pretrained: bool = False, **kwargs) -> VitMixerRelPos:
    """ VitMixerRelPos-Pico (Pi/16) - 128 dim """
    model_args = dict(patch_size=16, embed_dim=128, depth=12, num_heads=4)
    model = _create_vitmixer_relpos('vitmixer_relpos_pico_patch16_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model


@register_model
def vitmixer_relpos_tiny_patch16_224(pretrained: bool = False, **kwargs) -> VitMixerRelPos:
    """ VitMixerRelPos-Tiny (Ti/16) """
    model_args = dict(patch_size=16, embed_dim=192, depth=12, num_heads=3)
    model = _create_vitmixer_relpos('vitmixer_relpos_tiny_patch16_224', pretrained=pretrained, **dict(model_args, **kwargs))
    return model
